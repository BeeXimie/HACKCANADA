...
class FlaskSessionTransactionStore:
    async def get(self, key, options=None): return session.get(f"auth0_transaction_{key}")
    async def set(self, key, value, options=None): session[f"auth0_transaction_{key}"] = value
    async def delete(self, key, options=None): session.pop(f"auth0_transaction_{key}", None)

state_store = FlaskSessionStateStore()
transaction_store = FlaskSessionTransactionStore()
